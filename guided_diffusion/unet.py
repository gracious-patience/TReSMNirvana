...
class IQAUNetModel(nn.Module):
    ...
    def __init__(
        ...
        time_embed_dim = model_channels * 4
        self.time_embed = nn.Sequential(
            linear(model_channels * k, time_embed_dim),
            nn.SiLU(),
            linear(time_embed_dim, time_embed_dim),
        )
    ...
    def forward(self, x, timesteps, y=None):
        ...
        hs = []
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels).reshape([-1, self.model_channels * self.k]) )
